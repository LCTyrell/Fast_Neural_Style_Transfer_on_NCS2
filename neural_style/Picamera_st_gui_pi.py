...
def run_stylize():    
    if checkbox1.value==True:
        ...
        st_mrf.mrf("fns/styles_IR/rain_princess.xml",input_box2.value)
        
        tstop=time.time()
        print("Inference time : "+str(1000*(tstop-tstart))+" ms")
        
        #image_mrf = cv2.imread("st_image_0.bmp")
        #window_name = "out"
...
input_box1 = TextBox(dir_box, multiline=False, text="/home/pi/Documents", width="fill", align="left")
...
input_box2 = TextBox(dir_box2, multiline=False, text="/home/pi/Documents", width="fill", align="left", enabled=False)
...
text4 = Text(left_box,size=10, text="NCS2 : ", grid=[0,3])
checkbox2 = CheckBox(left_box, command=nbox, grid=[1,3])


text1 = Text(left_box,size=10, text="Device : ", grid=[0,0])
combo1 = Combo(left_box, options=["Picamera", "USB Camera", "File"], width=12, command=input_choice, grid=[1,0])
text2 = Text(left_box,size=10, text="Style : ", grid=[0,1])
combo2 = Combo(left_box,command=show_img, options=["Mosaic", "Candy", "Princess", "Udnie", "NCS2 MRF 13"], width=12, grid=[1,1])
